...
vec3 reflectedBRDF(Ray ray, Intersect impact){
    ...
    newRay.O = impact.t*ray.V + ray.O+newRay.V*2.0;
...
void main(void){
    ...
    const int msaa = 16; //Valeur possible 4 - 9 - 16 - 25 - 36

    vec3 colors[msaa];
    Ray rayons[msaa];

    for(int i = 0; i < int(sqrt(float(msaa))); i++){
        for(int j = 0; j < int(sqrt(float(msaa))); j++){
            rayons[ i*4 +j].V =
                normalize(
                    vec3(
                    cam_width*( gl_FragCoord.x+float(i)*1.0/sqrt(float(msaa)) -screenSize.x/2.0)/(screenSize.x/2.0),
                    cam_height*( gl_FragCoord.y+float(j)*1.0/sqrt(float(msaa)) -screenSize.y/2.0)/(screenSize.y/2.0),
                    cam_focal
                )
            );
            rayons[ i*4 +j].O = cam_origin;
        }
    }
    ...
    for (int i = 0; i < msaa ; i++){
        ...
        rayons[i].V = (cam_rotation * vec4(rayons[i].V,1.0) ).xyz;

        //lancement du rayon et retour d'un élément intersect au point d'impact
        Intersect impact = throw(rayons[i]);
        colors[i] = blendLights(impact, rayons[i]);

        //Gestion reflectance
        if (impact.mat.mirror > 0.0){
            vec3 BRDF2;
            BRDF2 = reflectedBRDF(rayons[i], impact);
            colors[i] = ( (1.0-impact.mat.mirror)*colors[i] + impact.mat.mirror*BRDF2);
        }

    }

    // Fait la moyenne des rayons
    vec3 BRDF;
    for(int i = 0; i < 16 ; i++){
        BRDF += colors[i];
    }
